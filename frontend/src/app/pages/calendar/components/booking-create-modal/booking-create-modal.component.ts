...
export class BookingCreateModalComponent {
  ...
  draft = signal<Partial<BookingCreate>>({
    status: 'Confirmed', adults: 1, children: 0,
  });
  ...
  isValid(): boolean {
    const d = this.draft();
    return !!(d.booking_id?.trim() && d.guest_name?.trim() && d.check_in && d.check_out && this.nights() > 0);
  }
  ...
  save(): void {
    ...
    const payload: BookingCreate = {
      booking_id: d.booking_id!, guest_name: d.guest_name!,
      check_in: d.check_in!, check_out: d.check_out!,
      status: d.status ?? 'Confirmed', nights: this.nights(),
      persons: (d.adults ?? 1) + (d.children ?? 0), adults: d.adults ?? 1, children: d.children ?? 0,
      price: d.price ?? null, charges: d.charges ?? null,
    ...
    this.bookingService.createBooking(payload).subscribe({
      next: created => { this.created.emit(created); this.saving.set(false); },
