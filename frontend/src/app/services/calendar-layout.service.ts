...
export class CalendarLayoutService {
  ...
  weekHeight(totalLanes: number): number {
    return this.DAY_HEADER_H + this.BAR_TOP_PAD +
           totalLanes * (this.BAR_HEIGHT + this.BAR_GAP) + 8;
  }
  ...
  barTop(laneIndex: number): number {
    return this.DAY_HEADER_H + this.BAR_TOP_PAD +
           laneIndex * (this.BAR_HEIGHT + this.BAR_GAP);
  }
  ...
  buildWeeks(
    ...
    for (let w = 0; w < calDates.length; w += 7) {
      ...
      const overlapping = bookings.filter(b =>
        b.check_in <= weekEndStr && b.check_out >= weekStartStr
      );
      ...
      const bars: WeekBar[] = rawBars.map(bar => ({ ...bar, laneIndex: laneRemap.get(bar.laneIndex)! }));
  ...
  private buildBar(
    ...
    const colStart = isCheckin
      ? weekDates.findIndex(wd => wd.iso === b.check_in)
      : 0;
    const colEnd = isCheckout
      ? weekDates.findIndex(wd => wd.iso === b.check_out)
      : 6;
