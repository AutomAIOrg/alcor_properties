...
class TestGetByBookingId:
    def test_returns_entity_when_found(self, sqlite_session):
        ...
        result = SQLAlchemyApartmentRepository(sqlite_session).get_by_booking_id("R180")

        assert isinstance(result, Apartment)
        assert result.booking_id == "R180"
        assert result.community == "Alta Entinas"

    def test_returns_none_when_not_found(self, sqlite_session):
        result = SQLAlchemyApartmentRepository(sqlite_session).get_by_booking_id("UNKNOWN")
...
class TestSearchTextFilters:
    ...
    def test_q_matches_community_case_insensitive(self, sqlite_session):
        ...
        assert results[0].booking_id == "R180"

    def test_q_matches_booking_id(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="R180")
        _insert_apartment(sqlite_session, booking_id="R221")
        ...
        assert len(results) == 1
        assert results[0].booking_id == "R221"

    def test_q_matches_owner_name(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="R180", owner_name="Katarzyna Tokarska")
        _insert_apartment(sqlite_session, booking_id="R221", owner_name="Juan Pérez")
    ...
    def test_address_field_filter_partial_match(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="R180", address="Calle Glaucio 15")
        _insert_apartment(sqlite_session, booking_id="R221", address="Avenida del Mar 3")
...
class TestSearchNumericFilters:
    def test_min_rooms_excludes_apartments_below_threshold(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="SMALL", rooms=1)
        _insert_apartment(sqlite_session, booking_id="LARGE", rooms=4)
        ...
        assert len(results) == 1
        assert results[0].booking_id == "LARGE"
    ...
    def test_max_rooms_excludes_apartments_above_threshold(self, sqlite_session):
        ...
        assert len(results) == 1
        assert results[0].booking_id == "SMALL"

    def test_min_max_rooms_combined(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="ONE", rooms=1)
        _insert_apartment(sqlite_session, booking_id="TWO", rooms=2)
        _insert_apartment(sqlite_session, booking_id="FOUR", rooms=4)
        ...
        assert len(results) == 1
        assert results[0].booking_id == "TWO"

    def test_min_max_occupants_combined(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="SMALL", total_occupants=2)
        _insert_apartment(sqlite_session, booking_id="MEDIUM", total_occupants=6)
        _insert_apartment(sqlite_session, booking_id="LARGE", total_occupants=10)
        ...
        assert len(results) == 1
        assert results[0].booking_id == "MEDIUM"
...
class TestSearchAvailabilityFilter:
    ...
    def test_available_apartment_returned_while_booked_one_excluded(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="BUSY")
        _insert_apartment(sqlite_session, booking_id="FREE")
        _insert_booking(
            sqlite_session,
            booking_id="BUSY",
            check_in=date(2026, 6, 3),
        ...
        assert len(results) == 1
        assert results[0].booking_id == "FREE"
...
class TestSearchOrdering:
    def test_results_are_ordered_by_booking_id_ascending(self, sqlite_session):
        _insert_apartment(sqlite_session, booking_id="R500")
        _insert_apartment(sqlite_session, booking_id="R100")
        _insert_apartment(sqlite_session, booking_id="R300")

        results = SQLAlchemyApartmentRepository(sqlite_session).search_apartments(
            ApartmentSearchFilters()
        )

        assert [r.booking_id for r in results] == ["R100", "R300", "R500"]
